...
export function WorkflowBuilder({ tenantId = 'default_tenant', onRefresh }: WorkflowBuilderProps) {
  ...
  useEffect(() => {
    ...
    const mockData = Array.from({ length: 15 }).map((_, idx) => ({
      id: `workflowbuilder_${idx + 1}`,
      name: `WorkflowBuilder Record #${idx + 1}`,
      code: `REF-${1000 + idx}`,
  ...
  return (
    <div className="glass-card rounded-2xl p-6 border border-slate-800 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-slate-100">WorkflowBuilder Portal</h3>
          <p className="text-xs text-slate-400 mt-0.5">Enterprise CRM component for WorkflowBuilder management</p>
        </div>
        <button
